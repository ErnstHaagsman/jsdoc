        ...
        onTagged: function(doclet, tag) {
            if (tag.value) {
                ...
                if (doclet.meta.code.type && /STRING|NUMBER|NAME|TRUE|FALSE/.test(doclet.meta.code.type)) {
                    ...
                    // toSource (even though it returns a string according to the documentation) returns an
                    // object which is incompatible with JSON.stringify; therefore the cast to string and eval
                    // the parentheses are required when running eval on javascript objects.
                    // See http://rayfd.me/2007/03/28/why-wont-eval-eval-my-json-or-json-object-object-literal/
                    doclet.defaultvalue = JSON.stringify(eval('('+String(doclet.meta.code.node.toSource())+')'));
                }
            }
        }
    })
